...
function gameFunctions(whatToDo) {
	...
	if(whatToDo == "resetGame"){
		...
		total_questionsAnswered = 0;
		total_correctlyAnswered = 0;
	...
	if(whatToDo == "start"){
		...
		$("#main-display").text("Begin!");
		setTimeout(function() {
			$("#main-display").text("");
		}, 1000);

		if(snail2_movement.length == 0){
			$("#snail-2-holder").addClass("hide");
		}else{
			$("#snail-2-holder").removeClass("hide");
			snail2_used = true;
			snailAI("moveSnail2");
		}

		if(snail3_movement.length == 0){
			$("#snail-3-holder").addClass("hide");
		}else{
			$("#snail-3-holder").removeClass("hide");
			snail3_used = true;
			snailAI("moveSnail3");
		}
	...
	if(whatToDo == "count"){
		if(time != 0){
    		...
    		if(position_p1 >= 100){
    			...
    			//place win and restart in row 1
	    		var $winGame = $("<div>");
				$winGame.html("<div>CONGRATULATIONS!<br>You Won!</div>");
				$winGame.attr("id","win-btn");
				$("#row-1").append($winGame);
				shakeThings($winGame, true);

				//place stats in row 2
				var $winStats = $("<div>");
				$winStats.html("<div>Total Questions: "+total_questionsAnswered+"</div>" +
					"<div>Correct Answers: "+total_correctlyAnswered+"</div>" + 
				"<div>Distance Traveled: "+position_p1+"</div>");
				$winStats.attr("id","win-stats");
				$("#row-2").append($winStats);
				shakeThings($winStats, true);

				//place try again button in row3
				setTimeout(function() {

					var $tryAgain = $("<button>");
					$tryAgain.html("<div>Try Again?</div>");
					$tryAgain.attr("id","win-btn");
					$("#row-3").append($tryAgain);
					shakeThings($tryAgain, true);
...
function questionSpawner(category, questionNumber, rowParent) {
	...
	var $qH = $("<div>", {"class": "question-holder"});
		...
		$sB.click(function(event) {
			...
			if(myStatus == "open" && !gameOver){

				total_questionsAnswered +=1;
				...
				//try and push time into p2 or p3
				if(!snail2_used){
...
function snailAI(whichSnail, whatToDo) {
	...
	if(whichSnail == "p1"){
		...
		$("#snail-player").css( "margin-left", position_p1 + "%" );
		$("#snail-player-trail").css( "width", (position_p1-0.2) + "%" );
	}
	if(whichSnail == "p2"){
